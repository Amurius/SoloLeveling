...
export const postSignIn = async ({ formulaire }: { formulaire: any }) => {
  if (formulaire.email.match(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+.[A-Z]{2,4}$/i)? true: false){
    if (formulaire.password.match(/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&*£§_-]).{8,}$/)? true: false){
      var pswHashed = pswHash.generate(formulaire.password)
      var date = new Date()
      var admin = 0
      const result = await excuteQuery({
        query: 'INSERT INTO users (us_nom,us_prenom,us_email,us_password,us_date_inscription, us_admin) VALUES (?,?,?,?,?,?)',
        values: [formulaire.nom, formulaire.prenom, formulaire.email, pswHashed, date, admin],
      });
      return {success : true}
    } else {
